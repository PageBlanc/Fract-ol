...
int mouse_hook(int mousecode,int x, int y, f_fract *fract)
{
	...
	if (mousecode == 4)
	{
		fract->panx = ((double)x / fract->zoom + fract->panx) - ((double)x / (fract->zoom * 1.5));
		fract->pany = ((double)y / fract->zoom + fract->pany) - ((double)y / (fract->zoom * 1.5));
		fract->zoom *= 1.5;
	}
	else if (mousecode == 5)
	{
		fract->panx = ((double)x / fract->zoom + fract->panx) - ((double)x / (fract->zoom * 0.5));
		fract->pany = ((double)y / fract->zoom + fract->pany) - ((double)y / (fract->zoom * 0.5));
		fract->zoom *= 0.5;
	}
	else if (mousecode == 1)
